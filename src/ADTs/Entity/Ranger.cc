...
Ranger::Ranger(double health, double energy, double attack, double armour, pair<int, int> position,
               ...
               vector<shared_ptr<Equipable>> equipables) : Player{health, energy, attack, armour, "Ranger", position, consumables, equipables} {}
...
vector<pair<int, int>> Ranger::specialMoves()
{
    ...
    if (auto mp = current_map_.lock())
    {
        ...
        else if (((position_.second + down) < mp->numRows()) &&
